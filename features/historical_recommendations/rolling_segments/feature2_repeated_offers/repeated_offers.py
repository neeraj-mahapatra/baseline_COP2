...
def process_consultora_data(consultora_history, rolling_segments, offer_metadata, reference_date, threshold, alpha=0.5, beta=0.5):
    """
    ...
    consultora_history = pd.merge(consultora_history, rolling_segments, on='Consultora_ID', how='left')
    print('No of rows that doesnt have rolling segments',consultora_history['rolling_segemnt'].isnull().sum())
    merged_data = pd.merge(consultora_history, filtered_data, on='Composite_Key', how='inner')

    # Step 4: Group and aggregate
    grouped_data = merged_data.groupby(['rolling_segemnt', 'Composite_Key']).agg(
...
if __name__ == "__main__":
    ...
    rolling_segments = pd.DataFrame({  # Example schema
        'Consultora_ID': [1, 2],
        'Country': ['PE', 'PE'],
        'camapign':['202304','202304'],
        'rolling_segemnt': ['Constantes 2','Constantes 2']
    })
    ...
    result = process_consultora_data(consultora_history,rolling_segments, offer_metadata, reference_date, threshold)
    print(result)
