...
class Transformer:
    ...
    def process_csv(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "rolling_segment" in df.columns and "price" in df.columns:
            df["total_spent"] = df.groupby("rolling_segment")["price"].transform("sum")
            df["purchase_count"] = df.groupby("rolling_segment")["product_id"].transform("count")
            df["unique_products"] = df.groupby("rolling_segment")["product_id"].transform("nunique")
